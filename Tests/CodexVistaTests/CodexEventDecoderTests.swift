...
final class CodexEventDecoderTests: XCTestCase {
    ...
    func testRepositoryBackedWorkspaceIdentityMergesLinkedWorktreeAndUsesCatalogName() throws {
        let mainPath = "/Users/example/work/CodexVista"
        let worktreePath = "/Users/example/.codex/worktrees/abc/CodexVista"
        let main = try XCTUnwrap(WorkspaceIdentity.resolve(
            rootPaths: [mainPath],
            repositoryIDsByRootPath: [mainPath: "repository-id"],
            displayName: "CodexVista Workspace"
        ))
    ...
    func testSessionUsesEmbeddedGitRepositoryWhenWorkingDirectoryNoLongerExists() throws {
        let session = #"{"type":"session_meta","payload":{"id":"thread-worktree","source":"cli","cli_version":"1.0.0","cwd":"/deleted/worktree/CodexVista","git":{"commit_hash":"abc123","repository_url":"git@github.com:ychp-ai/CodexVista.git"}}}"#
...
final class GitRepositoryIdentityResolverTests: XCTestCase {
    ...
    private func createRepository(at url: URL, content: String) throws {
        ...
        try runGit([
            "-C", url.path,
            "-c", "user.name=CodexVista Tests",
            "-c", "user.email=codexvista-tests@example.invalid",
            "commit", "-q", "-m", "fixture"
        ])
    }
    ...
    private func temporaryDirectory() -> URL {
        FileManager.default.temporaryDirectory
            .appending(path: "CodexVista-GitIdentityTests-\(UUID().uuidString)")
    }
}
