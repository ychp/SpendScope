...
- 在工作区排行、工作区详情、今日任务列表和任务详情中展示耗时；工作区耗时跟随今日、7 日、30 日和累计范围，进行中的回复计至最近一次刷新。
...
- **回复明细**：每次回复的时间、状态、耗时、模型及调用次数、Token、Skill 调用数和 Tool 调用数；子 agent 的 Token、模型及调用活动并入实际创建它的主回复，但生命周期状态仍由主回复自身事件决定，避免子 agent 完成时提前结束仍在运行的主任务；默认按时间倒序展示。
...
| 耗时 | 当前工作区范围内可见任务的回复生命周期累计时长；未结束的回复计至最近一次刷新，子 Agent 归并到主回复且不重复计时，不代表人类实际投入时间 |
