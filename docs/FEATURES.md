...
- 展示今日 Token 总量，以及 5 小时 / 7 天剩余额度和重置时间。
...
- 用环形图展示 5 小时与 7 天剩余额度、重置时间和官方额度同步状态。
- 额度因代理规则无法刷新时，在看板中显示明确提醒。
- 汇总今日、近 7 日、近 30 日和累计 Token。
- 每个周期同时展示未缓存输入、缓存输入、可见输出和推理 Token。
- 工具栏支持立即刷新、打开设置、看板置顶，以及将看板收起为仅显示额度的紧凑窗口。
- 支持加载、正常、空数据、过期、失败和格式不兼容等状态。

![SpendScope 详细看板](images/spendscope-dashboard.png)

### 用量趋势与日历

- 趋势图支持 7 天和 30 天范围，展示区间总计与日均 Token。
- 悬浮趋势节点可查看当日总 Token 和四类 Token 构成。
...
- **任务明细**：任务名称、最后活动时间、回复数、工作区内占比和 Token；支持搜索，并可按“最近消息”或“用量”排序。
- **回复明细**：每次回复的时间、状态、耗时、模型、Token、Skill 调用数和 Tool 调用数，默认按时间倒序展示。
- **调用悬浮详情**：悬浮回复行时，在详情窗口外侧展示该回复的四类 Token，以及完整 Skills / Tools 名称和各自调用次数；面板会根据屏幕空间选择右、左、下或上方。
...
- 随后立即增量读取新增或变化的 Codex 记录，并尝试刷新一次官方账户额度。
- 默认每 60 秒检查一次新增用量。
- 用量指纹变化后将额度标记为待刷新；默认每 120 秒按需检查额度，没有变化时跳过读取。
- 自动刷新可以在设置中关闭。

### 官方额度读取

- 通过本机已安装的 Codex 可执行文件启动 `app-server --stdio`，读取官方账户额度。
- 最近一次成功结果会缓存在 SpendScope 数据库中，实时读取失败时可继续展示有效缓存。
- 尚无官方缓存时，才使用 rollout 中可识别的额度观测作为后备。
- 额度缺失、无效或已过期时不会推断为 100%。
- 可选择“仅在检测到系统代理时刷新额度”；未检测到代理时保留待刷新状态，并在弹窗和看板中提示。

### 手动操作

- **立即刷新**：增量读取上次刷新后新增或变化的本机记录，并按当前代理规则刷新额度。
...
![SpendScope 设置页面](images/spendscope-settings.png)
...
| 5 小时 / 7 天额度 | 最近一次有效官方账户额度；无官方缓存时使用本地 rollout 观测后备 |
...
聚合统计、官方额度缓存、来源状态和导入检查点保存在：

```text
~/Library/Application Support/SpendScope/SpendScope.sqlite
```

SpendScope 不修改或删除 Codex 原始文件和数据库。只有以下功能会访问应用外部能力：

- 官方额度刷新调用本机 Codex `app-server --stdio`；
- 软件更新检查和下载访问本项目的 GitHub Releases。

这两类操作都不会上传 SpendScope 保存的统计数据库、会话内容或项目数据。
