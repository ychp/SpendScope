...
- 展示每日趋势、月度日历、Skills / Tools 调用排行、项目用量和模型用量。
...
回复级统计以 `turn_context.turn_id` 为归属键：当前累计快照算出的每一份增量都绑定到当时的 turn，仍然使用上述累计差值，避免一轮回复中多次 `token_count` 重复相加。生命周期事件提供开始、完成、中断或回滚时间，查询层据此计算状态与耗时；没有明确 turn 的历史增量保留在任务总量中，并标记为无法归属。
...
看板按四个时间范围展示项目 Token、占比、项目数量和总量。项目行可展开为任务列表，默认按最后一条消息时间倒序，也可切换为 Token 用量排序；任务还可继续展开为按时间倒序的回复列表，展示四类 Token、总量、生命周期状态和耗时。列表名称来自只读索引并仅在内存中使用，缺失时显示匿名任务标识；回复正文始终不进入采集链路。
...
- 累计：全部已导入事件。
- 趋势和日历：按日汇总四类 Token。
- 模型占比、活动排行、项目与任务用量，以及按模型聚合的 Token 构成与 API 等值费用估算。
- 5 小时与 7 天最新可信额度。
...
| 2026-07-30 | 项目任务支持展开查看每次回复的四类 Token、状态和耗时（迁移 v10） |
