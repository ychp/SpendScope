...
| `ProjectDetailView` | 展示项目概览、目录用量、任务明细、回复明细，以及任务级和回复级完整 Skill / 工具调用列表 |
...
看板按四个时间范围展示项目 Token、占比、项目数量、涉及目录数量和总量。排行行显示配置根目录数量及本期实际产生用量的目录；点击项目行会打开独立详情窗口。今日任务页直接从今日项目排行展开可见任务，进行中状态优先，相同状态按最后更新时间倒序；点击任务会打开包含四类 Token、回复数、项目相关目录和回复明细的独立详情窗口。原看板在详情打开期间忽略鼠标事件，拖动详情窗口时原看板同步移动。

项目详情分为三个标签：

- **概览**：展示项目 Token、全局占比、根目录与本期目录用量、任务与回复数量、四类 Token 构成和近 7 日趋势；任务列表只在任务明细中展示。
