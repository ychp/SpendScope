...
模型费用使用显式的公开 API 标准价格目录，分别对未缓存输入、缓存输入、可见输出和推理输出计算；推理 Token 按输出价格估算。该数值不等同于 Codex 订阅账单，也不包含无法从聚合数据识别的长上下文、缓存写入和工具调用附加费。`codex-auto-review` 等没有公开独立价格的内部路由保留为未定价，不纳入费用总额。
