...
官方额度刷新只调用本机 Codex app-server，不读取认证文件；软件更新功能访问本项目 GitHub Releases。两类请求都不携带 SpendScope 保存的 Token、会话或项目统计。
