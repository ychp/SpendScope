...
- `appearance.skin` 独立持久化经典／水墨皮肤，未知值回退经典；`appearance.colorScheme` 保存经典皮肤的跟随系统、浅色和深色偏好；水墨在共享外观容器中强制浅色，不覆盖保存值，切回经典时恢复此前偏好。`CodexVistaPalette` 集中定义两款现有皮肤的三组色板，经典颜色按系统外观解析，水墨始终解析为浅色色板，切换皮肤时重建窗口的临时展示状态，共享数据与统计状态保持不变。经典额度环刻度、水墨纸纹、山水远舟及额度笔触由确定性的静态 Canvas 绘制，装饰不接收点击，也不进入辅助功能树；额度条长度来自实际剩余额度，山水装饰与数据无关，仅绘制于额度区下沿且不改变布局尺寸。皮肤预览卡沿用既有枚举与偏好键。
- 看板置顶。
