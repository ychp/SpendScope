...
- 支持手动刷新、用量 60 秒自动刷新、额度 120 秒按需刷新和从本机原始记录全量重建。
...
flowchart LR
    ...
    C --> D["最小事件解码"]
    L["本机 Codex app-server<br/>官方账户额度"] --> G
    D --> E["Token / 额度 / 套餐标准化"]
    D --> F["会话与活动归约"]
...
| `ProjectDetailView` | 展示工作区概览、目录用量、任务明细、回复明细与完整 Skill / 工具调用列表 |
...
账户额度还有一条独立的只读路径：应用从 Codex.app、ChatGPT.app、`~/.local/bin`、Homebrew 或 `/usr/local/bin` 中发现本机 Codex 可执行文件，以 `app-server --stdio` 启动短生命周期子进程，并调用 `account/rateLimits/read`。该请求只提交 SpendScope 客户端名称与 Bundle 版本，不携带本地统计、会话或项目数据；执行超时为 10 秒。
...
官方额度刷新只调用本机 Codex app-server，不读取认证文件；用户可选择在刷新前检查 macOS 系统代理状态，未检测到 HTTP、HTTPS、SOCKS 或自动代理时跳过本次额度读取、保留待刷新标记，并在看板与状态栏弹窗显示原因。后续检测到代理，或用户关闭代理检查后，提示会清除。该选项默认关闭，关闭时不会读取系统代理状态。软件更新功能访问本项目 GitHub Releases。两类请求都不携带 SpendScope 保存的 Token、会话或项目统计。
...
回复级统计以 `turn_context.turn_id` 为归属键：当前累计快照算出的每一份增量都绑定到当时的 turn，仍然使用上述累计差值，避免一轮回复中多次 `token_count` 重复相加。生命周期事件提供开始、完成、中断或回滚时间，查询层据此计算状态与耗时；Skills / Tools 活动同样按 `(thread_id, turn_id)` 归入对应回复，并按调用名称聚合次数。没有明确 turn 的历史 Token 保留在任务总量中，没有明确 turn 的活动不进入回复调用明细，二者都不会被猜测归属。
...
额度展示优先使用 `account/rateLimits/read` 返回的官方账户窗口，并将同一次响应按窗口原子写入 `account_rate_limit_cache`；刷新失败时可继续读取最近一次缓存。官方缓存不存在时，查询层仍可使用 rollout 中的额度快照。官方结果或缓存已过期时不回退猜测为满额。
...
- **任务明细**：支持搜索任务，默认按最后一条消息时间倒序，也可切换为 Token 用量排序。
- **回复明细**：按时间倒序展示四类 Token 总量、时间、状态、耗时、模型以及本次回复的 Skill / 工具调用总数。

鼠标进入回复行时，`ProjectDetailView` 将对应回复传给 `ProjectDetailWindowController`。控制器使用透明、非激活、忽略鼠标事件的子 `NSPanel`，在工作区详情窗口外侧展示完整 Skill / 工具名称、聚合次数和四类 Token；面板优先位于右侧，空间不足时依次尝试左侧、下方和上方，并作为子窗口跟随详情窗口移动。调用种类全部渲染，内容超过可用高度时在面板内部滚动，单个长名称自动换行而不截断。
...
5. 发布首份真实快照。
6. 后台补齐历史数据并再次发布。
7. 启动时立即执行一次用量增量刷新，并尝试读取一次最新官方额度；该行为不受自动刷新开关影响，但遵循可选的“额度刷新检查代理”设置。
8. 启动后默认每 60 秒执行一次用量增量刷新，并比较累计 Token 指纹；发生变化时标记额度待刷新。
9. 每 120 秒检查一次额度刷新标记；需要时读取一次最新额度，否则跳过。
10. 用户手动刷新时，无论用量是否变化，都在用量刷新后立即尝试一次额度刷新；开启代理检查后，未检测到系统代理时跳过额度读取并保留待刷新标记。

`CodexImporter` 与数据客户端都串行化操作；并发刷新会合并或排队，不同时扫描同一来源。手动刷新、自动刷新和全量重建共享同一统计口径。全量重建通过异步进度回调上报清理、发现、逐文件导入和最终查询阶段；文件发现完成后以所选 rollout 总数为分母，每处理一个文件更新一次确定进度，`DashboardStore` 作为全局状态所有者向设置页发布进度。
...
| `account_rate_limit_cache` | 本机 Codex app-server 返回的官方账户额度与观测时间 |
...
- 本订阅周期：用户设置第一次订阅时间后，以该本地日历日期和时刻为锚点逐月计算；月末日期自动落在目标月的最后一天，查询范围从本周期起点精确到当前时刻。
- 累计：全部已导入事件。
- 趋势和日历：按日汇总四类 Token。
...
- 默认与最小窗口约束为 `920 × 620`。
- 不使用页面级 `ScrollView`。
- 顶部复合卡左侧展示 7 天额度，右侧以 2×2 四宫格展示今日、7 日、30 日、累计；设置第一次订阅时间后，在四宫格上方增加本订阅周期汇总条。
...
- 回复调用悬浮卡使用窗口外侧子面板，完整展示全部 Skill / 工具名称和次数，不遮挡列表；面板随工作区详情窗口移动。
...
- 来源健康状态。
- 第一次订阅时间；保存于本机偏好设置，用于逐月计算本订阅周期 Token。
- 自动检查和自动下载更新。
...
- `DashboardStore` 用量/额度拆分刷新、按需额度标记、并发合并与加载状态转换。
- 官方账户额度缓存的原子替换、重启恢复、字段校验与失败降级。
- 额度提醒阈值、去重、权限和投递失败。
