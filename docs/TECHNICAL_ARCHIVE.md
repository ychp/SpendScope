...
看板按四个时间范围展示工作区 Token、占比、工作区数量、涉及目录数量和总量。排行行显示配置根目录数量及本期实际产生用量的目录；点击工作区行会打开独立详情窗口。今日任务页直接从今日工作区排行展开可见任务，进行中状态优先，相同状态按最后更新时间倒序；点击任务会打开包含四类 Token、回复数、工作区相关目录和回复明细的独立详情窗口。原看板在详情打开期间忽略鼠标事件，拖动详情窗口时原看板同步移动。
...
- 顶部标签可切换今日任务、用量趋势、Skills / Tools、工作区用量和模型用量。
