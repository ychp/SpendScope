...
- 展示每日趋势、月度日历、Skills / Tools 调用排行、项目用量、回复级调用明细和模型用量。
...
├── Features/
│   ├── Dashboard/       看板、趋势日历、活动/项目/模型排行和费用明细
│   ├── MenuBar/         菜单栏状态项和弹窗
│   └── Settings/        设置窗口
├── Models/              UI 与查询层共享模型
...
| `DashboardQueryService` | 生成四周期、趋势、额度、活动/项目/模型排行和费用估算 |
| `SessionQueryService` | 提供不含对话内容的会话筛选与新鲜度查询 |
| `ProjectDetailView` | 展示项目概览、任务明细、回复明细与完整 Skill / 工具调用列表 |
| `ProjectDetailWindowController` | 管理锁定原看板的项目详情窗口，以及窗口外侧的非激活悬浮面板 |
...
索引读取使用 SQLite 只读模式和短 busy timeout，只提取线程 ID、rollout 路径、来源、模型、时间、归档状态、工作目录及子代理关系等安全字段。项目任务列表还会读取 `threads.name` / `threads.title` 作为页面展示名称；该字段会规范为空白折叠后的单行文本、最多 120 个字符，只在内存中参与看板组装，不进入 SpendScope 数据库或日志。以 `The following is the Codex` 开头的系统上下文模板不会作为名称透传：根据安全的来源元数据回退为 `命令权限检查` 或 `Codex 子任务`，无法识别时继续使用匿名任务标识。索引异常会降级文件发现，而不是阻断全部导入。
...
看板按四个时间范围展示项目 Token、占比、项目数量和总量。排行表本身不展开内容，点击项目行会打开独立项目详情窗口；原看板在详情打开期间忽略鼠标事件，拖动详情窗口时原看板同步移动。

项目详情分为三个标签：

- **概览**：展示项目 Token、全局占比、任务与回复数量、四类 Token 构成、近 7 日趋势和任务用量列表。
- **任务明细**：支持搜索任务，默认按最后一条消息时间倒序，也可切换为 Token 用量排序。
- **回复明细**：按时间倒序展示四类 Token 总量、时间、状态、耗时、模型以及本次回复的 Skill / 工具调用总数。

鼠标进入回复行时，`ProjectDetailView` 将对应回复传给 `ProjectDetailWindowController`。控制器使用透明、非激活、忽略鼠标事件的子 `NSPanel`，在项目详情窗口外侧展示完整 Skill / 工具名称、聚合次数和四类 Token；面板优先位于右侧，空间不足时依次尝试左侧、下方和上方，并作为子窗口跟随详情窗口移动。调用种类全部渲染，内容超过可用高度时在面板内部滚动，单个长名称自动换行而不截断。

任务名称来自只读索引并仅在内存中使用，缺失时显示匿名任务标识；内部“命令权限检查”任务从项目列表中过滤。回复正文始终不进入采集链路。
...
`CodexImporter` 与数据客户端都串行化操作；并发刷新会合并或排队，不同时扫描同一来源。手动刷新、自动刷新和全量重建共享同一统计口径。
...
| `usage_events` | 去重后的四类 Token 增量、回复归属、模型、套餐和项目标识 |
...
- 累计：全部已导入事件。
- 趋势和日历：按日汇总四类 Token。
- 模型占比、活动排行、项目/任务/回复用量、回复级 Skill / 工具调用，以及按模型聚合的 Token 构成与 API 等值费用估算。
- 5 小时与 7 天最新可信额度。
...
- 顶部标签可切换用量趋势、Skills / Tools、项目用量和模型用量。
- Skills 排行先合并带命名空间的细分项，再按汇总次数排序；包含细分项的排行行可通过悬浮面板查看完整构成。
- 项目用量排行保持单层表格；项目详情通过独立窗口展示，并包含概览、任务明细和回复明细三个标签。
- 任务列表提供搜索和“最近活动 / Token 用量”排序；回复列表直接展示状态、耗时、模型、Skill 与工具调用总数。
- 回复调用悬浮卡使用窗口外侧子面板，完整展示全部 Skill / 工具名称和次数，不遮挡列表；面板随项目详情窗口移动。
...
- 今日、7 日、30 日、累计、趋势、额度、活动/项目/模型排行和费用估算。
- 项目任务排序、回复 Token 与生命周期归属、回复级 Skill / 工具聚合，以及无 `turn_id` 活动不被误归属。
