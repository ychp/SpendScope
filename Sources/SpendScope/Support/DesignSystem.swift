...
struct SpendScopeAppearanceContainer<Content: View>: View {
...
enum SpendScopeGlassEmphasis {
    case standard
    case strong
}

struct SpendScopeGlassGroup<Content: View>: View {
    let spacing: CGFloat?
    private let content: Content
...
struct SpendScopeGlassSurface: ViewModifier {
    let padding: CGFloat
    let cornerRadius: CGFloat
    let emphasis: SpendScopeGlassEmphasis
    ...
    func body(content: Content) -> some View {
        ...
        if #available(macOS 26.0, *) {
            content
                ...
                .shadow(color: SpendScopeTheme.dashboardGlow, radius: 20)
                .shadow(
                    color: SpendScopeTheme.dashboardShadow.opacity(shadowOpacity),
                    radius: 12,
                    y: 4
                )
        } else {
    ...
    private var nativeGlassTint: Color? {
        if emphasis == .strong {
            return SpendScopeTheme.accent.opacity(colorScheme == .dark ? 0.075 : 0.04)
        }
    ...
    private var nativeFrostTint: Color {
        if emphasis == .strong {
            return SpendScopeTheme.glassTintStrong.opacity(colorScheme == .dark ? 0.34 : 0.28)
        }
        return SpendScopeTheme.glassTint.opacity(colorScheme == .dark ? 0.26 : 0.22)
    }
            ...
            .fill(
                LinearGradient(
                    gradient: Gradient(stops: [
                        .init(
                            color: SpendScopeTheme.glassHighlight.opacity(
                                colorScheme == .dark ? 0.22 : 0.36
                            ),
                            location: 0
                        ),
                        .init(
                            color: SpendScopeTheme.glassTint.opacity(
                        ...
                        .init(
                            color: SpendScopeTheme.glassEdge.opacity(0.12),
                            location: 1
                        )
                    ]),
                    startPoint: .topLeading,
    ...
    private func glassEdgeOverlay(shape: RoundedRectangle) -> some View {
        ZStack {
            shape.stroke(SpendScopeTheme.dashboardBorder, lineWidth: 1)
            shape.stroke(
                LinearGradient(
                    gradient: Gradient(stops: [
                        .init(color: SpendScopeTheme.glassHighlight, location: 0),
                        .init(color: Color.clear, location: 0.46),
                        .init(color: SpendScopeTheme.glassEdge, location: 1)
                    ]),
                    startPoint: .topLeading,
                    endPoint: .bottomTrailing
                ),
...
extension View {
    func dashboardCard(padding: CGFloat = 18) -> some View {
        modifier(SpendScopeGlassSurface(
            padding: padding,
            cornerRadius: 16,
            emphasis: .standard,
            shadowOpacity: 0.62
