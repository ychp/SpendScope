...
- **使用排行**：查看 Skills、Tools 的调用次数，以及项目、任务和每次回复的 Token 用量明细。
...
| 项目用量 | 优先按项目名汇总；同名目录通过 Git 仓库指纹识别，属于同一仓库时合并统计；任务可按最近消息或用量排序，并可继续展开查看每次回复的四类 Token、状态和耗时 |
...
回复级用量只保存 Codex 提供的回复标识、生命周期时间、状态、模型和 Token 计数，用于归属统计与计算耗时；不会读取或保存回复正文。
