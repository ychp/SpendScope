...
- **原生 macOS 体验**：提供经典与水墨两种皮肤，经典支持浅色、深色和跟随系统，水墨固定为浅色，支持看板置顶、状态栏内容定制、数据源健康状态、全量重建进度和 GitHub Releases 更新检查。
...
现有**水墨皮肤**采用雾白纸面、淡墨山水与远舟、笔触额度条、宋体标题、衬线数字与朱砂印记，仅支持浅色。前往「设置 → 外观 → 皮肤 → 水墨」切换。

![水墨皮肤设计预览](docs/design/codexvista-ink-light.png)

配色和交互说明见[外观设计说明](docs/design/README.md)。

以下界面截图保留更名前 v1.3.0 的界面，仅供布局参考；当前应用名称已统一为 CodexVista，外观已更新为经典／水墨皮肤，数据会随本机 Codex 使用记录变化；关联目录、刘海摘要设置和模型排行布局已进一步调整，以本文功能说明为准。
