...
- **使用排行**：查看 Skills、Tools 的调用次数，以及项目、任务和每次回复的 Token、状态、耗时、模型与调用明细。
...
看板集中展示当前额度、今日/近 7 日/近 30 日/累计 Token，以及用量日历和趋势图。顶部标签可以切换到 Skills / Tools、项目用量和模型用量排行；项目排行可打开独立详情窗口查看项目概览、任务和回复，模型排行同时提供 Token 构成、公开 API 标准价格与费用明细。费用仅为基于本地 Token 的 API 等值估算，不代表 Codex 实际账单，未公开定价的内部模型不会被强行套价。
...
项目用量页保留紧凑排行，不直接在表格中展开内容。点击项目行的“查看详情”会打开独立项目详情窗口，并锁定原看板；拖动详情窗口时原看板会同步移动。详情窗口包含：

![SpendScope 项目用量排行，展示项目占比、任务数、回复数和 Token](docs/images/spendscope-project-usage.png)

- **概览**：展示项目 Token、全局占比、任务与回复数量、四类 Token 构成、近 7 日趋势，以及可搜索、可切换“最近活动 / Token 用量”排序的任务列表。
- **任务明细**：集中查看任务名称、最后活动时间、回复数、项目内占比和 Token；默认按最后一条消息时间倒序，也可手动切换为用量排序。
- **回复明细**：按时间倒序展示每次回复的 Token、时间、状态、耗时、模型，以及本次回复的 Skill 和工具调用总数。
- **调用悬浮详情**：鼠标停在回复行上时，在项目详情窗口外侧显示完整的 Skills、工具名称与各自调用次数，同时展示该回复的四类 Token。外置面板不会遮挡原列表，会跟随详情窗口移动；屏幕右侧空间不足时自动尝试左侧、下方或上方。

#### 项目概览

![SpendScope 项目详情概览，展示 Token 构成、近 7 日趋势和任务用量](docs/images/spendscope-project-overview.png)
...
内部“命令权限检查”任务不会出现在项目任务列表中。没有明确回复归属的历史 Token 仍计入任务和项目总量，但不会被错误分配给某次回复。
...
3. 点击“打开看板”，查看用量趋势、用量日历、Skills / Tools 排行、项目用量，以及按模型统计的 Token 和 API 等值费用；在项目排行中点击“查看详情”可以继续查看任务、回复和每次回复的 Skill / 工具调用。
...
| 推理 | 模型内部推理使用的 Token |
| Skills / Tools | 本机记录中识别出的调用次数；项目回复明细还会按线程与回复标识归属，展示每次回复调用过的完整 Skill / 工具名称和次数 |
| 项目用量 | 优先按项目名汇总；同名目录通过 Git 仓库指纹识别，属于同一仓库时合并统计；项目详情中的任务可按最近消息或用量排序，回复按时间倒序展示四类 Token、状态、耗时、模型和调用明细 |
...
打开项目详情后会显示 Codex 页面上的任务名称。该名称仅从最新 `state_*.sqlite` 的 `threads.name` / `threads.title` 字段只读获取、临时保留在内存中，不会写入 SpendScope 数据库、日志或上传。系统上下文模板不会直接作为名称展示：可识别的 Guardian / 子任务会显示安全的任务类型名称，其余不可用名称回退为匿名任务标识；内部“命令权限检查”任务不进入项目任务列表。
