...
- **额度常驻菜单栏**：直接查看 5 小时与 7 天额度、已用或剩余比例，以及重置倒计时。
- **Token 全景看板**：汇总今日、近 7 日、近 30 日和累计用量，拆分未缓存输入、缓存输入、可见输出与推理 Token。
- **趋势与用量日历**：通过 7/30 天趋势图、月度热力日历和悬浮明细定位使用高峰。
...
- **原生 macOS 体验**：支持看板收起、置顶、状态栏内容定制、数据源健康状态、全量重建进度和 GitHub Releases 更新检查。
...
4. 打开“设置”，按需调整状态栏、额度提醒、自动刷新、更新检查和看板置顶。

应用会先展示上一次成功保存的统计，再从本机 Codex 数据中增量补充记录并同步官方额度。
...
SpendScope 不会修改或删除 Codex 原始记录。刷新官方额度时会调用本机 Codex `app-server --stdio`；启用更新检查时会访问本项目的 GitHub Releases。这些操作都不会上传 SpendScope 的统计数据库。详细说明见 [完整功能说明：数据与隐私](docs/FEATURES.md#数据与隐私边界)。
