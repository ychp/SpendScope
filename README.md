...
- **标准化层**：解码 Token、额度、模型、套餐、会话状态和 Skills / Tools 调用，将累计 Token 转为可聚合增量。
- **存储与查询层**：使用应用自有 SQLite 保存去重事件、小时聚合、额度快照、来源状态和项目身份，再生成看板快照。
- **展示与系统层**：共享 `DashboardStore` 驱动状态栏、弹窗、看板和设置，并管理额度通知及 GitHub Release 更新检查。

启动时会先展示上一次成功保存的统计，再优先导入近期记录，最后在后台补齐历史。重复刷新、应用重启和会话归档不会重复累计同一事件。
...
设置页完整包含看板置顶、状态栏展示、额度口径、重置倒计时、用量提醒、数据来源、自动刷新、数据重建和软件更新。Codex 套餐与其他计费方式区域当前用于信息说明，不进行费用或账单计算。
...
├── Features/
│   ├── Dashboard/        Token 看板、用量日历、活动排行和项目用量
│   ├── MenuBar/          状态栏渲染与弹窗
│   └── Settings/         设置窗口
├── Models/               看板与查询共享模型
├── Resources/            App、状态栏和 Codex 图标
└── Support/              偏好、更新、提醒、格式化和视觉系统
...
`.github/workflows/unsigned-release.yml` 提供手动触发的 GitHub Actions 工作流。输入版本标签后，它会：

1. 在 macOS runner 上运行测试；
2. 构建同时支持 `arm64` 和 `x86_64` 的 Universal Release App；
3. 校验两种架构并制作拖拽安装式 DMG；
4. 生成对应版本的源码 ZIP、tar.gz 和 SHA-256 校验文件；
