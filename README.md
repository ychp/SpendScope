...
- **使用排行**：查看 Skills、Tools 的调用次数，以及工作区、目录、任务和每次回复的 Token、状态、耗时、模型与调用明细。
...
- **存储与查询层**：使用应用自有 SQLite 保存去重事件、小时聚合、额度快照、官方额度缓存、来源状态、工作区和项目身份，再生成周期、工作区、活动和模型排行。
...
看板集中展示当前额度、今日/近 7 日/近 30 日/累计 Token，以及用量日历和趋势图。顶部标签可以切换到 Skills / Tools、工作区用量和模型用量排行；工作区排行可打开独立详情窗口查看目录构成、任务和回复，模型排行同时提供 Token 构成、公开 API 标准价格与费用明细。费用仅为基于本地 Token 的 API 等值估算，不代表 Codex 实际账单，未公开定价的内部模型不会被强行套价。
...
工作区用量页以 Codex `turn_context.workspace_roots` 的根目录组合区分工作区，并优先使用 Codex `local-projects` 中的工作区名称；已归档工作区只要名称曾被本机元数据记录，也会保留名称并继续出现在排行中。Git 主目录和关联 worktree 会按仓库身份合并为同一个根目录，避免同一工作区被重复拆行；同名单目录工作区的历史记录若共享相同项目路径或 Git 仓库身份，也会合并，兼容早期记录缺少仓库标识的情况。只是名称相同但目录身份不同的工作区不会合并；经确认的历史临时工作区可以通过本地派生 ID 别名并入目标工作区，而且别名在全量重建后仍会保留。同一个目录仍可同时出现在不同的根目录组合中，其 Token 会分别归属。旧记录缺少 `workspace_roots` 时，会以会话工作目录建立单独的“工作目录推测”工作区。点击工作区行的“查看详情”会打开独立详情窗口，并锁定原看板；拖动详情窗口时原看板会同步移动。详情窗口包含：

![SpendScope 工作区用量排行，展示工作区占比、目录、任务数、回复数和 Token](docs/images/spendscope-project-usage.png)

- **概览**：展示工作区 Token、全局占比、根目录与本期涉及目录、任务与回复数量、四类 Token 构成、近 7 日趋势，以及可搜索、可切换“最近活动 / Token 用量”排序的任务列表。
- **任务明细**：集中查看任务名称、最后活动时间、回复数、工作区内占比和 Token；默认按最后一条消息时间倒序，也可手动切换为用量排序。
- **回复明细**：按时间倒序展示每次回复的 Token、时间、状态、耗时、模型，以及本次回复的 Skill 和工具调用总数。
- **调用悬浮详情**：鼠标停在回复行上时，在工作区详情窗口外侧显示完整的 Skills、工具名称与各自调用次数，同时展示该回复的四类 Token。外置面板不会遮挡原列表，会跟随详情窗口移动；屏幕右侧空间不足时自动尝试左侧、下方或上方。
...
![SpendScope 项目任务明细，支持搜索和最近活动或 Token 用量排序](docs/images/spendscope-task-details.png)

#### 回复与调用详情

![SpendScope 回复明细与窗口外调用详情，展示完整 Skills、Tools 和 Token 构成](docs/images/spendscope-reply-activity-detail.png)

内部“命令权限检查”任务不会出现在工作区任务列表中，也不计入主排行、顶部卡片或目录行的任务数、回复数和最后活动时间，保证这些位置使用同一可见任务口径；它产生的 Token 仍计入真实工作区用量。没有明确回复归属的历史 Token 仍计入任务和工作区总量，但不会被错误分配给某次回复。

### 设置

![SpendScope 完整设置页面，展示全部配置选项](docs/images/spendscope-settings.png)

设置页完整包含看板置顶、状态栏展示、额度口径、重置倒计时、用量提醒、数据来源、自动刷新、额度刷新代理检查、数据重建和软件更新。全量重建时会显示清理、扫描、逐文件抓取和生成统计结果等阶段；发现本地记录后展示真实的“已处理 / 总文件数”进度条。Codex 套餐与其他计费方式区域用于说明订阅和 API Key 的计费差异；模型费用估算统一在详细看板的“模型用量”标签中展示。
...
3. 点击“打开看板”，查看用量趋势、用量日历、Skills / Tools 排行、工作区用量，以及按模型统计的 Token 和 API 等值费用；在工作区排行中点击“查看详情”可以继续查看目录、任务、回复和每次回复的 Skill / 工具调用。
...
| 推理 | 模型内部推理使用的 Token |
| Skills / Tools | 本机记录中识别出的调用次数；工作区回复明细还会按线程与回复标识归属，展示每次回复调用过的完整 Skill / 工具名称和次数 |
| 工作区用量 | 按每次 turn 的 `workspace_roots` 根目录组合归属，名称优先取 Codex 工作区元数据；归档名称会安全保留，Git worktree 与主目录按仓库身份合并。同名单目录记录只有在共享项目路径或仓库身份时才合并；仅名称相同不会合并。同一目录被多个不同根目录组合使用时仍分别统计。缺少根目录组合的旧记录按会话工作目录单独标记为“推测”。工作区内再按目录与 Git 仓库身份汇总，并展示任务、回复、四类 Token、状态、耗时、模型和调用明细 |
...
SpendScope 只读取统计所需的本机 Codex 字段，例如 Token 计数、额度窗口、模型、套餐、会话来源、工作目录、工作区根目录组合、Codex 工作区名称以及 Skills / Tools 调用标识。工作区根目录只用于生成稳定哈希和匹配工作区名称；识别主目录与 worktree 时还会读取 Git remote、根提交或公共 Git 目录。数据库只保存这些派生哈希、工作区名称、安全展示名和根目录数量，不保存完整工作区路径、原始 Git 地址或项目路径。

打开工作区详情后会显示 Codex 页面上的任务名称。该名称优先从最新 `state_*.sqlite` 的 `threads.name` / `threads.title` 字段只读获取、临时保留在内存中，不会写入 SpendScope 数据库、日志或上传。系统上下文模板不会直接作为名称展示：子任务可使用安全的代理昵称、角色或任务类型生成回退名称，Guardian 显示为“命令权限检查”并从工作区任务列表过滤，其余不可用名称回退为匿名任务标识。SpendScope 不会读取第一条用户消息生成名称。

回复级用量只保存 Codex 提供的回复标识、生命周期时间、状态、模型和 Token 计数，用于归属统计与计算耗时。经白名单识别的 Skill / 工具活动只保存类型、名称、时间、线程与回复归属，用于汇总调用次数；不会读取或保存回复正文、工具参数或工具输出。
...
├── Features/
│   ├── Dashboard/        Token 看板、用量日历、活动/工作区/模型排行和费用明细
│   ├── MenuBar/          状态栏渲染与弹窗
│   └── Settings/         设置窗口
├── Models/               看板与查询共享模型
...
每次运行都会在构建前和启动前关闭已有的 SpendScope 实例，再启动本次构建产物；即使两个本地验证流程发生重叠，也只会保留最后启动的一个应用实例。
...
- 四周期、趋势、额度、活动/工作区/模型排行和 API 等值费用查询；
- 会话状态、新鲜度和归档关系；
